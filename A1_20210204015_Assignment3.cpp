...
void set_val(string name,string id_type, string data_type, string scope, string value) {
    int i = add(name);
...
void modify_stream(string filename) {
    ifstream f(filename);
    ...
    while (f >> tok) {
        if (tok.rfind("[id ", 0) == 0) {
            string name = tok.substr(4, tok.size() - 5);
            if (name == "z") {
                zcount++;
