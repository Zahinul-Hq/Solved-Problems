...
const int N = 1e3 + 7;
struct chash {
    int operator()(pair<ll,ll> x) const { return x.first * 31 + x.second; }
};
gp_hash_table<pair<ll, ll>, int, chash> mp;

int base1 = 137, base2 = 277, mod1 = 957499, mod2 = 999983;
    ...
    while (test_cases--)
    {
        ...
        for(int l = 1; l <= s.size(); l++){
        	for(int i = 0; i < s.size() - l + 1 ; i++){
        		...
        		pair<int,int> p = get_hash(i, i + l - 1, s.size());
        		st.insert(p);
