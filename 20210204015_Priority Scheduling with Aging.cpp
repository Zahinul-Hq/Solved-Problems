...
struct CompareProcess {
    bool operator()(const Process& p1, const Process& p2) {
        if (p1.Curr_Priority != p2.Curr_Priority) {
            return p1.Curr_Priority < p2.Curr_Priority;
        }
...
void Aging(int AT){
	...
	while(!temp_pq.empty()){
		Process p = temp_pq.top();
		if(p.Response_Time == -1 && p.Arrival_Time != AT){
			p.Curr_Priority += Y;
		}
		pq.push(p);
		temp_pq.pop();
...
void Show_Priority_Values(int time){
	temp_pq = pq;

	cout << "Time(s) Process Current_Priority" << endl;

	while(!temp_pq.empty()){
		Process p = temp_pq.top();
		cout <<"  " << time<< "       " << p.Process_Name << "             " << p.Curr_Priority << endl;
		temp_pq.pop();
...
void Tabular_Form(){
	...
	cout << "Process   Arrival   Finish   Turn_Around   Waiting   Response" << endl;
	...
	for(int i = 0; i < n; i++){
		...
		cout << "   " << answer[i].Process_Name << "         " << answer[i].Arrival_Time << "        " << answer[i].Finishing_Time << "         " << TAT << "          " << Waiting_time << "          " << response_time << endl;
	}
...
void Input(){
	...
	// cout << "Enter Priority of the Processes" << endl;
	// cout << "Higher The Number Higher The Priority" << endl;
	// for(int i = 0; i < n; i++){
	// 	cin >> PT[i];
	// }
	...
	for(int i = 0; i < n; i++){
		...
		//pr.Priority = pr.Curr_Priority = PT[i];
		pr.Response_Time = -1;
		arr.push_back(pr);
	}
}
...
int main()
{
	...
	int curr_AT = curr_process.Arrival_Time;
	...
	for(int time = 0; !pq.empty(); time++){
		...
		//curr_process.Curr_Priority = curr_process.Priority;
		curr_AT = curr_process.Arrival_Time;
